<template>
  <div class="ArticleDetails">
    <div class="article">
      <h2 class="title">{{ policy.policyTitle }}</h2>
      <p class="info">
        <span>发布机构 ： {{ policy.pubAgency }}  </span>
<!--        <span style="width: 10em;"/>-->
        <span>创建时间 ： {{ policy.pubTime }}</span>
<!--        <span style="width: 10em;"/>-->
        <span>更新时间 ： {{ policy.updateTime }}</span>
      </p>
      <div class="content">
        <div v-for="(text, key) in policy.policyBody.split(/\s+/)" :key="key">{{ text }}</div>
      </div>
      <div class="ChangeBtn" v-if="isAdmin">
...
export default {
  ...
  data() {
    return {
      ...
      policy: {
        "policyId": "100068247",
        "policyTitle": "安徽省人民政府办公厅关于做好疫情防控期间返程、复工、返校人员管理工作的通知",
        "policyGrade": "省级",
